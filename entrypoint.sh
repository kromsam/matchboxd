...
run_script() {
    /usr/src/app/venv/bin/python -m matchboxd_scraper # Use absolute path to Python
...
echo "$CRON_SCHEDULE /usr/src/app/venv/bin/python -m matchboxd_scraper >> /var/log/matchboxd.log 2>&1" >>/etc/cron.d/matchboxd
