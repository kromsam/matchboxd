...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const letterboxdList = url.pathname;
...
const cityjsonURL = getJSONurl('city_json');
const jsonURL = getJSONurl('json');
...
function renderData (sortingMode) {
  ...
  if (sortingMode === 'film') {
    ...
    sortedDates.forEach(date => {
      ...
      showings.sort((a, b) => a.showing.time_start.localeCompare(b.showing.time_start));

      const card = document.createElement('div');
      card.className = 'col-md-4';

      const showingsList = showings.map(({ film, showing }) => `
                  <div class="showing">
                      <div class="showing-item">
                          <p><strong><a href="${film.url}" target="_blank">${film.title}</a></strong><br>
                          ${showing.additional_info ? `<span class="additional-info">${showing.additional_info}</span>` : ''}</p>
      ...
      card.innerHTML = `
                  <div class="card">
                      <div class="card-body">
                          <h2 class="card-title">${formatDateToDutch(date)}</h2>                                    
